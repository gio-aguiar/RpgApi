       ...
       [HttpPost("Habilidade")]
        ...
        public async Task<IActionResult> DisputaEmGrupoAsync(Disputa d)
        {
            try
            {
                ...
                int qtdPersonagensVivos = personagens.Count(p => p.PontosVida > 0); // Corrigido para contar os personagens vivos
                ...
                while (qtdPersonagensVivos > 1)
                {
                    ...
                    qtdPersonagensVivos = personagens.Count(p => p.PontosVida > 0); // Atualiza a quantidade de personagens vivos

                    // Seleciona um próximo ataque após um pequeno intervalo
